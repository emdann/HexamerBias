...
dir='/hpc/hub_oudenaarden/edann/crypts_bs/met_calls'
# dir="/hpc/hub_oudenaarden/edann/reikVSnla/met_extraction"
files=[]
for file in os.listdir(dir):
    if fnmatch.fnmatch(file, 'cov2c*.cov.gz'):
...
with gzip.open(files[0], "rb") as f:
	for line in f:
		...
		cov_dict['\t'.join([aline[i] for i in [0,1,2]])]=[int(i) for i in aline[3:5]]
...
for file in files[1:]:
	with gzip.open(file,'rb') as f:
		for line in f:
			...
			cov_dict['\t'.join([aline[i] for i in [0,1,2]])][0]+= int(aline[3])
			cov_dict['\t'.join([aline[i] for i in [0,1,2]])][1]+= int(aline[4])
