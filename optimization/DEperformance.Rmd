...
ctcf.prof <- load.profile('~/mnt/edann/hexamers/DEoptimization/ctcf_cov/predicted_cov_bestmat/highcov.random.42.500regtest9_ctcf_kmersFC_pop30_its200.bestMat.coverage.artCov.CTCF.profile.txt')
...
```{r}
opt.dir <- '~/mnt/edann/hexamers/DEoptimization/even_cov/'
score.even.files <- list.files(opt.dir, pattern = '.+even_pop30_its100.+rho.txt', full.names = TRUE)
...
```{r}
originalpop.even.files <- list.files(opt.dir, pattern = '.+even_pop30_its100.+originalPop.csv', full.names = TRUE)
