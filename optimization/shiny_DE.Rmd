...
```{r}
dir <- "/Users/user/mnt/edann/hexamers/DEoptimization/ctcf_cov/"
samples <- gsub(pattern = '.DE.+', replacement = '',list.files(dir)[grep(list.files(dir), pattern = 'matrix')])
```
  ...
  ui = fluidPage(
      sidebarLayout(
        sidebarPanel(
              ...
              sliderInput("it", label="Iteration", min = 1, max = 100, value = 1, animate = animationOptions(interval = 300, loop = F))
        )
      ,
        mainPanel(
              plotOutput('probmat'),
