...
Load data
```{r}
predicted.bw <- '~/mnt/edann/pred_coverage_primer_batch_D3R/evenNreads/random.42.mm10.noOvsD3R_dens_0.25_0.25_0.25_0.25evenNreads_predcoverage.artCov.bw'
experimental.bw <- '~/mnt/edann/primer_conc_VAN2493/D3R_tr2_R1_bismark_bt2.deduplicated.srt.bw'
```

```{r}
spear.d3r <- map_dbl(track.list.D3R, corr.real)
rand.d3r <- randomize(track.list.D3R)

data.frame(sample='d3r_WGS', spearman.rho=spear.d3r) %>%
  bind_rows(., data.frame(sample='random', spearman.rho=rand.d3r)) %>%
  ggplot(., aes(sample, spearman.rho)) +
  geom_boxplot(outlier.alpha = 0.2, varwidth = T) +
  theme_classic() +
  ggsignif::geom_signif(comparisons = list(c("d3r_WGS", "random")), 
              map_signif_level=TRUE) +
  theme(axis.title=element_text(size=20), 
        axis.text=element_text(size=16), 
        strip.text=element_text(size=20), 
...
```{r}
setwd('~/mnt/edann/VAN2591/cov_tracks/')
cele.new <- make.predVSexp.track(predicted.bw = 'WBcel235.random.42.srt.noOvs.noChrcElegans_handMixNew_predcoverage.artCov.bw',
                                 experimental.bw = '../CG-BS-pbat-bulk-1xPreamp-noBS-cElegans-whole-handMixNew_lmerged_R1.srt.bw')

human.new <- make.predVSexp.track(predicted.bw = 'hg38.random.42.srt.noOvshuman_handMixNew_predcoverage.artCov.bw',
                                 experimental.bw = '../CG-BS-pbat-bulk-1xPreamp-noBS-human-APKS-handMixNew_lmerged_R1.srt.bw')

```

```{r}
spear.cele <- compare.spear.real(cele.new, name='cElegans_WGS')
spear.human <- compare.spear.real(human.new, name='human_WGS')
...
cov.cor.df <- as.data.frame(spear.cele$spear) %>%
  rownames_to_column(var='id') %>%
  inner_join(., mean.df) %>%
  mutate(spear.group = cut(spear.cele$spear, breaks = seq(-1,1,by = 0.25), include.lowest = TRUE)) %>% 
  mutate(spear.group = cut(spear.cele$spear, breaks = c(-1,seq(0,1,by = 0.25)), include.lowest = TRUE)) 
...
ggplot(cov.cor.df, aes(y=mean.cov, x=spear.group)) +
  ...
  xlab('Spearman corr. coefficient') + ylab('Mean normalized coverage') +
  theme(axis.title = element_text(size = 20), axis.text = element_text(size=10), title = element_text(size=22)) +
  geom_signif(comparisons = list(levels(cov.cor.clean$spear.group)[c(3,5)],
                                 levels(cov.cor.clean$spear.group)[c(2,5)],
                                  levels(cov.cor.clean$spear.group)[c(2,4)],
                                 levels(cov.cor.clean$spear.group)[3:2],
                                 levels(cov.cor.clean$spear.group)[c(3,4)]
                                 ),
