...
eps.pt.handMixOld <- exp(predict(model.epsilon, data.frame(genomecov=filter(gencov.BS, grepl(smp, pattern = paste0(input.pattern, '.+handMixOld')))$genomecov)))
eps.pt.handMixNew <- exp(predict(model.epsilon, data.frame(genomecov=filter(gencov.BS, grepl(smp, pattern = paste0(input.pattern, '.+handMixNew')))$genomecov)))
