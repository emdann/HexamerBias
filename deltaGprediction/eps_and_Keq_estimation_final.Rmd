...
Downsampling experiments to better fit the curve
```{r}
pt.bootstrap.files<- list.files("~/mnt/edann/VAN2423_onePreamp/bootstrap/", pattern = '.ptCounts', full.names = T)
bootstrap.pt.tabs <- mclapply(pt.bootstrap.files, load.pt.data, mc.cores = detectCores())
...
human.keqs <- compute.keqs(human.all.df, eps.human, filter.pt = 10)
zf.keqs <- compute.keqs(zf.all.df, eps.zf, filter.pt = 10)
