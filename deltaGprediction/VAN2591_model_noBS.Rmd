...
```{r}
p.usage.zf.T <- compute.primer.usage(zf.T.all.df)
p.usage.zf.G <- compute.primer.usage(zf.G.all.df)
p.usage.zf.R <- compute.primer.usage( pt.tabs$`dRerio-whole-handMixNew`)

inner_join(p.usage.zf.R, p.usage.zf.T, by=c('hex'), suffix=c('.G', '.T')) %>%
  mutate(nuc=sapply(hex, prevalent_nucleotide)) %>%
  ggplot(., aes(p.usage.G, p.usage.T, color=nuc)) + geom_point(alpha=0.3)

summarise(p.usage.zf.G, p.usage=sum(p.usage))
summarise(p.usage.zf.T, p.usage=sum(p.usage))
summarise(p.usage.zf.R, p.usage=sum(p.usage))
```
