...
def de(fobj, seq_len, mut=0.8, crossp=0.7, popsize=20, its=1000, cores=10):
    ...
    for i in range(its):
        for f,trial,j in workers.imap_unordered(de_mutation, [ (fobj,j,pop,seq_len,popsize,mut,crossp) for j in range(popsize)]):
            if f < fitness[j]:
                fitness[j] = f
                pop[j] = trial
...
def de_mutation(params):
    ...
    '''
    fobj,j,pop,seq_len,popsize,mut,crossp = params
...
list(de(test_function, 6, popsize=20, its=1000, cores=10))
